...
- git branch is a lightweight pointer pointing to one commit container, and that is why branch creation and descrution is fast and cheap in git and it makes branching git's killer feature
- files in a repository can be one of the four states, untracked, unmodified (commited), modified, staged
...
- rebase local changes before pushing to clean up the work, but never rebase anything that is already pushed to somewhere (do not rebase commits that exist outside your local repository which people may have based their work on).
